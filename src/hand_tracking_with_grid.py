...
def draw_grid():
    """Draw a sci-fi holographic grid beneath the hands."""
    glDisable(GL_DEPTH_TEST)  # Disable depth testing to ensure grid is always visible

    glColor3f(0.0, 0.5, 1.0)  # Neon blue grid
    glLineWidth(1)

    grid_size = 1.0  # Controls how far the grid extends
    step = 0.1  # Spacing between lines

    glBegin(GL_LINES)
    for i in np.arange(-grid_size, grid_size + step, step):
        # Vertical lines
        glVertex3f(i, -0.5, -grid_size)
        glVertex3f(i, -0.5, grid_size)

        # Horizontal lines
        glVertex3f(-grid_size, -0.5, i)
        glVertex3f(grid_size, -0.5, i)
    glEnd()
...
def draw_hand_skeleton():
    """Render both hands' skeletons with a sci-fi grid background."""
    ...
    # Render hand skeleton first
    hand_colors = [(0.0, 0.8, 1.0), (1.0, 0.2, 0.2)]  # Blue & Red Hands
    for index, hand_landmarks in enumerate(hand_landmarks_list):
        color = hand_colors[index % len(hand_colors)]
        ...
        for lm in hand_landmarks:
            draw_sphere(lm, radius=0.03)
        glEnable(GL_LIGHTING)

    # Now draw the grid after the hands have been rendered
    draw_grid()
